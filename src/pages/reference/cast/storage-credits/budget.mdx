...
Budget is transaction-local transient state, so a standalone read reflects the
default rather than a value set by an earlier `set-budget` transaction.

:::terminal

```bash
$ cast storage-credits budget --help
```

```txt
Usage: cast storage-credits budget [OPTIONS] <ACCOUNT>
