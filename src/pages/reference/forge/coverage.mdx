...
Options:
      --report <REPORT>
          ...
          - debug
          - bytecode
          - attribution: JSON report mapping each test to the source items it
            covers
      ...
      --report-file <PATH>
          ...
          Used only when a single file report is requested. If not specified,
          the report will be stored in the root of the project.
...
Test options:
  -j, --threads <THREADS>
      ...
      --symbolic-timeout <SECONDS>
          SMT solver timeout in seconds; also bounds symbolic invariant
          exploration
...
Compatibility:
  ...
  or
  execution modes such as `--json`, `--junit`, `--list`, `--debug`, flame
  profiles,
  symbolic artifact replay, showmap replay, brutalization, or mutation testing.
  Use
