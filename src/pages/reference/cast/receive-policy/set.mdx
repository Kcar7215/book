...
Create the sender and token-filter policies referenced here with `cast tip403
create`.

:::terminal

```bash
$ cast receive-policy set --help
```

```txt
Usage: cast receive-policy set [OPTIONS] <SENDER_POLICY_ID> <TOKEN_FILTER_ID>
...
Options:
      --recovery-authority <ADDRESS>
          Address authorized to recover held receipts. Defaults to originator
          recovery
          
          [default: 0x0000000000000000000000000000000000000000]

      --preview
          Print the calldata and receive-policy warning without sending a
          transaction
