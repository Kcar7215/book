...
```bash
$ cast keychain policy add-call --help
```

```txt
Usage: cast keychain policy add-call [OPTIONS] --target <TARGET> --selector <SELECTOR> <KEY_ADDRESS>
...
Options:
      --root-account <ADDRESS>
          Root account address. Required when the key is not present in the
          local keys.toml
