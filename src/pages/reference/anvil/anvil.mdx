...
Options:
  ...
  -m, --mnemonic <MNEMONIC>
      ...
      --mixed-mining
          Enable mixed mining mode. Blocks are mined on a timer (set by
          `--block-time`), but also whenever a transaction is submitted.
          Requires `--block-time` to be set
