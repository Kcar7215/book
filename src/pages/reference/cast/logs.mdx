...
Options:
      ...
      --query-size <BLOCKS>
          Split the query into chunks of this many blocks to work around
          provider range/result limits.
          
          When omitted, the range is queried in a single request. Pass a value
          (e.g. `10000`) to fetch the logs in `query-size`-block chunks instead.
