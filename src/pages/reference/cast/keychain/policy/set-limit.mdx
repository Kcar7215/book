...
```bash
$ cast keychain policy set-limit --help
```

```txt
Usage: cast keychain policy set-limit [OPTIONS] --token <TOKEN> --amount <AMOUNT> <KEY_ADDRESS>
...
Options:
      --token <TOKEN>
          Token address, numeric TIP-20 token id, or known Tempo fee-token
          symbol
      ...
      --period <PERIOD>
          ...
          The current AccountKeychain update entrypoint cannot change periods,
          so non-zero values are rejected.
