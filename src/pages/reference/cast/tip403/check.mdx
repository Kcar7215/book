...
```bash
$ cast tip403 check --help
```

```txt
Usage: cast tip403 check [OPTIONS] <POLICY_ID> <ADDRESS>
...
Options:
      --role <ROLE>
          Role to evaluate (defaults to the transfer check). Role variants
          require T2+
          
          [possible values: sender, recipient, mint-recipient]
