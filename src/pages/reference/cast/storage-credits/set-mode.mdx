...
The mode only applies within the transaction that sets it; batch it with the
storage operations it should govern.

:::terminal

```bash
$ cast storage-credits set-mode --help
```

```txt
Usage: cast storage-credits set-mode [OPTIONS] <MODE>
...
Arguments:
  <MODE>
          ...
          Possible values:
          - refund:   Pay creation cost upfront, then settle credits as a refund
            at end of transaction
          - preserve: Pay creation cost upfront and keep freed credits instead
            of spending them
          - direct:   Spend existing credits synchronously; selecting this sets
