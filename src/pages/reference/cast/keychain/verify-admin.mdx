...
secp256k1 signature). Returns true for the root key or an active admin key.
`hash` should already be domain-separated by the caller.

:::terminal

```bash
$ cast keychain verify-admin --help
```

```txt
Usage: cast keychain verify-admin [OPTIONS] <ACCOUNT> <HASH> <SIGNATURE>
