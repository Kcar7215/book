...
The budget only applies within the transaction that sets it; batch it with the
storage operations it should govern.

:::terminal

```bash
$ cast storage-credits set-budget --help
```

```txt
Usage: cast storage-credits set-budget [OPTIONS] <CREDITS>

Arguments:
  <CREDITS>
          Maximum number of credits the caller may spend in `direct` mode this
          transaction
