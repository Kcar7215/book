...
```txt
Usage: cast keychain authorize [OPTIONS] <KEY_ADDRESS> [KEY_TYPE] [EXPIRY]
...
Options:
      ...
      --scope <SCOPE>
          ...
          TARGET alone allows all calls. `TARGET:transfer,approve` restricts to
          those selectors. `TARGET:transfer@0x123` restricts selector to
          specific recipients

      --scopes <SCOPES_JSON>
          Call scope restrictions as a JSON array. Format:
          `[{"target":"0x...","selectors":["transfer"]}]` or
      ...
      --witness <WITNESS>
          ...
          For `--admin`, the `authorizeAdminKey` precompile always takes a
          witness; omitting this flag submits `bytes32(0)` (which fails if that
          witness is already burned).

      --admin
          Authorize a T6 admin access key via `authorizeAdminKey`
          (key-management only).
          
          Admin keys may authorize/revoke other keys but cannot carry an expiry,
          spending limits, or call scopes. The account is the signing
          (precompile caller) account.
