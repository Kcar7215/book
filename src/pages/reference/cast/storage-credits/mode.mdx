...
Mode is transaction-local transient state, so a standalone read reflects the
default rather than a value set by an earlier `set-mode` transaction.

:::terminal

```bash
$ cast storage-credits mode --help
```

```txt
Usage: cast storage-credits mode [OPTIONS] <ACCOUNT>
